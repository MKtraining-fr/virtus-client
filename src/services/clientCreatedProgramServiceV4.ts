    ...
    for (const program of programs) {
      ...
      if (sessions && sessions.length > 0) {
        for (const session of sessions) {
          ...
          // Créer un map pour accéder rapidement aux infos d'exercices
          const exercisesMap = new Map(
            exercisesInfo?.map((ex: any) => [ex.id, ex]) || []
          );

          // Construire les exercices au format WorkoutExercise
          const workoutExercises: WorkoutExercise[] = (sessionExercises || []).map((ex: any) => {
            totalExercises++;
            
            const exerciseInfo = exercisesMap.get(ex.exercise_id);
