...
export const renderAxes = (
  ...
  const yAxisElements = yTicks.map((tick, i) => {
    ...
    if (yMax > yMin) {
      yValue =
        margin.top + (height - margin.top - margin.bottom) * (1 - (tick - yMin) / (yMax - yMin));
    }
    return React.createElement(
      'g',
      { key: `y-tick-${i}`, className: 'text-gray-400 dark:text-gray-400' },
  ...
  const xAxisElements = data
    .map((d, i) => {
      ...
      if (i === 0 || i === data.length - 1 || i === Math.floor(data.length / 2)) {
        return React.createElement(
          'g',
          { key: `x-tick-${i}`, className: 'text-gray-500 dark:text-gray-400' },
          React.createElement(
            'text',
            { x: x, y: height - margin.bottom + 15, textAnchor: 'middle', fontSize: '10' },
            d.date
          )
        );
      }
  ...
  const yAxisLabel = React.createElement(
    'text',
    {
  ...
  const xAxisLabel = React.createElement(
    'text',
    {
      x: width / 2,
