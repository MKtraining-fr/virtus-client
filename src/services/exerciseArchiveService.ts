...
export async function archiveExercise(
  ...
  try {
    ...
    const { error: archiveError } = await supabase.from('exercise_archives').insert({
...
export async function restoreExercise(
  ...
  try {
    ...
    const { error: restoreError } = await supabase.from('exercises').insert(archive.exercise_data);
 ...
 */
export async function deleteArchivedExercise(
  archiveId: string
): Promise<{ success: boolean; error?: string }> {
  try {
    const { error } = await supabase.from('exercise_archives').delete().eq('id', archiveId);
...
export async function getArchivedExercises(
  userId: string
): Promise<{ data: ExerciseArchive[] | null; error?: string }> {
  try {
