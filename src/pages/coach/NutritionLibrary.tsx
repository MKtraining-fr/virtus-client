    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={() => navigate('/app/nutrition/createur')}>
                    Nouveau plan / recette
                </Button>
            </div>
                        ...
                        myPlans.map(plan => (
                            <Card key={plan.id} className="flex flex-col">
                               <div className="p-6 flex-grow">
                                 ...
                                 <p className="text-sm text-gray-500 mt-1">{plan.weekCount} semaine(s) · {plan.daysByWeek[1]?.length || 0} jours/semaine</p>
                               </div>
                               <div className="bg-gray-50 p-4 flex justify-end space-x-2">
                                    <Button variant="secondary" size="sm" onClick={() => navigate(`/app/nutrition/createur?editPlanId=${plan.id}`)}>Modifier</Button>
                                    <Button size="sm" onClick={() => handleOpenAssignModal(plan)}>Assigner</Button>
                               </div>
                            </Card>
                        ))
                    ) : (
                        ...
                        myRecipes.map(recipe => {
                            ...
                            return (
                                <Card key={recipe.id} className="flex flex-col">
                                   <div className="p-6 flex-grow cursor-pointer" onClick={() => handleViewRecipe(recipe)}>
                                     <h3 className="text-lg font-semibold text-gray-900">{recipe.name}</h3>
                                     <p className="text-sm text-gray-500 mt-1">
                                       {Math.round(macros.calories)} kcal | P:{Math.round(macros.protein)}g G:{Math.round(macros.carbs)}g L:{Math.round(macros.fat)}g
                                     </p>
                                   </div>
                                   <div className="bg-gray-50 p-4 flex justify-end space-x-2">
                                        <Button variant="secondary" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/app/nutrition/createur?editRecipeId=${recipe.id}`)}}>Modifier</Button>
                                        <Button variant="danger" size="sm" onClick={(e) => { e.stopPropagation(); handleDeleteRecipe(recipe.id)}}>Supprimer</Button>
                                   </div>
                                </Card>
                            )
                        })
            ...
            {isAssignModalOpen && planToAssign && (
                 <Modal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} title={`Assigner "${planToAssign.name}"`}>
                        ...
                        <div>
                            ...
                            <ul className="list-disc list-inside space-y-1">
                                {selectedRecipe.items.map(item => (
                                    <li key={item.id}>{item.food.name} - {item.quantity}{item.unit}</li>
                                ))}
                            </ul>
                        </div>

                        {selectedRecipe.steps && selectedRecipe.steps.length > 0 && selectedRecipe.steps.some(s => s && s.trim() !== '') && (
                            <div>
                                <h4 className="font-semibold text-lg mb-2 text-primary">Préparation</h4>
                                <ol className="list-decimal list-inside space-y-2">
                                    {selectedRecipe.steps.map((step, index) => (
                                        step && step.trim() !== '' && <li key={index}>{step}</li>
                                    ))}
                                </ol>
                            </div>
                        )}
                            ...
                            <h4 className="font-semibold text-lg mb-2 text-primary">Macros Totales</h4>
                             <div className="p-3 bg-gray-100 rounded-lg text-center">
                                <p className="text-sm text-gray-600">
                                   <span className="font-bold">{Math.round(calculateMacros(selectedRecipe.items).calories)} kcal</span> | 
                                   P: <span className="font-medium">{Math.round(calculateMacros(selectedRecipe.items).protein)}g</span> | 
                                   G: <span className="font-medium">{Math.round(calculateMacros(selectedRecipe.items).carbs)}g</span> | 
                                   L: <span className="font-medium">{Math.round(calculateMacros(selectedRecipe.items).fat)}g</span>
                                </p>
                            </div>
                        </div>
                    </div>
