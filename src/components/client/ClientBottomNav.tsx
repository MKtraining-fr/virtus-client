...
const ClientBottomNav: React.FC = () => {
  ...
  const hasNewMessage = useMemo(() => {
    ...
    return messages.some(m => m.clientId === user.id && m.senderId === user.coachId && !m.seenByClient);
  }, [user, messages]);
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex flex-col items-center justify-center space-y-1 w-full transition-colors duration-200 ${
              isActive ? 'text-primary' : 'text-gray-500 dark:text-client-subtle hover:text-gray-800 dark:hover:text-client-light'
            }`
          }
        >
