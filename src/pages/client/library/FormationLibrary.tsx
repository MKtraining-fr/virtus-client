...
const FormationCard: React.FC<{ formation: ClientFormation }> = ({ formation }) => {
  ...
  const commonClasses =
    'block bg-client-card rounded-lg p-4 cursor-pointer hover:bg-primary/20 transition-colors group w-full text-left';
  ...
  if (formation.type === 'link') {
    return (
      <a href={formation.url} target="_blank" rel="noopener noreferrer" className={commonClasses}>
        ...
        <p className="text-sm text-client-subtle mt-1 truncate">Cliquez pour ouvrir le lien</p>
      </a>
    );
  }
  ...
  if (formation.type === 'file') {
    return (
      <button onClick={handleDownload} className={commonClasses}>
        ...
        <p className="text-sm text-client-subtle mt-1 truncate">{formation.fileName}</p>
      </button>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate(-1)}
          className="p-2 bg-client-card rounded-full text-client-light hover:bg-primary/20"
        >
      ...
      {accessibleFormations.length > 0 ? (
        ...
        <div className="text-center py-16 bg-client-card rounded-lg">
          ...
          <p className="text-client-subtle mt-1">
            Votre coach ne vous a pas encore donné accès à une formation.
          </p>
        </div>
      )}
    </div>
