...
const ClientDashboard: React.FC = () => {
    ...
    const program = user?.assignedPrograms?.[0] || {
        name: 'Mon Programme',
        weekCount: 8,
        sessionsByWeek: { '1': [{ id: 1, name: 'Séance 1', exercises: [] }, { id: 2, name: 'Séance 2', exercises: [] }, { id: 3, name: 'Séance 3', exercises: [] }]}
    };
    const currentWeek = user?.programWeek || 1;
    const sessionProgress = user?.sessionProgress || 0;
    const sessionsForCurrentWeek = program.sessionsByWeek[currentWeek] ?? program.sessionsByWeek[1] ?? [];
                ...
                <div className="bg-primary/10 p-4 rounded-lg">
                    ...
                    <div className="mt-4 flex justify-between items-end">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-client-light">{currentWeek}<span className="text-lg text-client-subtle">/{program.weekCount}</span></p>
                        </div>
                        <div>
                            <p className="text-sm text-client-subtle">Séance</p>
                            <p className="text-2xl font-bold text-client-light">{sessionProgress}<span className="text-lg text-client-subtle">/{sessionsForCurrentWeek.length}</span></p>
                        </div>
