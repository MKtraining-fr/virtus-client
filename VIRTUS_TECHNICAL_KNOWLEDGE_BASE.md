...
## Intervention #13 - Système de Suivi des Performances et Projections (Janvier 2026)
...
L'objectif était de créer un système complet de suivi des performances de musculation permettant aux clients et coachs de :
- Visualiser les projections de charges sur différentes plages de répétitions (1-15 reps)
- Suivre l'évolution du 1RM dans le temps
- Déterminer le profil nerveux (Force/Endurance/Équilibré)
- Saisir manuellement des performances
- Synchroniser automatiquement les performances des séances validées

### Problèmes Rencontrés et Solutions

| Problème | Cause | Solution |
| :--- | :--- | :--- |
| **Interface avec trop de cartes** | Design initial avec grille de cartes pour chaque exercice | Remplacé par barre de recherche + vignette unique avec croix pour effacer |
| **Pas de persistance entre onglets** | État local non partagé | Création du hook `useFormPersistence` avec localStorage |
| **Perte de données au changement de fenêtre** | Pas de sauvegarde automatique | Hook `useFormPersistence` avec sauvegarde sur `visibilitychange` et `beforeunload` |
| **Exercices identiques non différenciables** | Plusieurs "Tirage vertical pronation" sans distinction | Ajout de l'équipement entre parenthèses dans toutes les listes déroulantes |
| **Tableau des projections vide** | Projections non générées automatiquement | Génération automatique des projections 1-15 reps à partir du 1RM |
| **Graphiques d'évolution vides** | Mauvais nom de champ (`exerciseId` vs `exercise_id`) | Correction du filtre pour utiliser `exercise_id` |
| **Imports supabase incorrects** | Chemin `../../lib/supabase` au lieu de `../../services/supabase` | Correction des imports dans tous les fichiers |

### Commits Réalisés

| Commit | Description |
| :--- | :--- |
| `c68e487` | feat: Système complet de tracking des performances et projections |
| `a800f36` | fix: Corriger les imports supabase (lib -> services) |
| `0d5930b` | refactor: Restaurer le design à 3 onglets pour le suivi des performances |
| `67e4b22` | revert: Restaurer la première version originale du système de performances |
| `a1749c7` | feat: Ajouter barre de recherche au-dessus de la vignette dans Projections |
| `88ed2b8` | feat: Améliorer la recherche et la vignette dans Projections |
| `79da9cb` | feat: Implémenter le système hybride de suivi des performances |
| `d936a09` | fix: Persistance sélection exercice, équipement dans listes déroulantes |
| `4145662` | fix: Corrections multiples - sélection par défaut, graphiques, persistance |
| `8ff88b5` | fix: Corrections tableau des projections et persistance de la sélection |
| `c6ed0c2` | style: Limiter la hauteur du tableau des projections avec scroll interne |
| `428715f` | feat: Réorganiser Objectif et Conditions en deux sous-sections |
| `dbe5f55` | feat: Ajouter la section Objectif et Conditions sur l'interface client |
...
- `client_exercise_records` : Stocke les performances manuelles et synchronisées (poids, reps, 1RM calculé, source)
- `client_exercise_projections` : Stocke les projections calculées pour chaque exercice

**Fonctions SQL:**
- `calculate_one_rm(weight, reps)` : Calcule le 1RM avec la formule de Brzycki
- `sync_session_performance_to_records()` : Trigger qui synchronise les performances des séances validées

**Trigger:**
- Déclenchement automatique sur `client_sessions` quand `status = 'completed'`
- Récupère les performances de `client_exercise_performance`
- Calcule le 1RM et insère dans `client_exercise_records` avec `source = 'session'`

#### 2. Système Hybride de Suivi des Performances

**Principe:**
- **1RM Actuel** : Basé sur la dernière performance enregistrée
- **Record Personnel (PR)** : Meilleur 1RM historique
- **Tendance** : Comparaison avec les performances précédentes (↑ progression, ↓ régression, → stable)

**Seuils de tendance:**
- ↑ Progression : +2% ou plus
- ↓ Régression : -2% ou moins
- → Stable : entre -2% et +2%

#### 3. Interface Utilisateur

**Composants créés:**
- `PerformanceSection.tsx` : Conteneur principal avec navigation par onglets et sous-sections
- `ProjectionsDisplay.tsx` : Affichage des projections, 1RM, PR, profil nerveux
- `PerformanceCharts.tsx` : Graphiques d'évolution du 1RM et charge/répétitions
- `PerformanceEntry.tsx` : Formulaire de saisie manuelle des performances
- `ExerciseSelector.tsx` : Sélecteur d'exercice avec recherche
- `BilanInfoDisplay.tsx` : Affichage des informations du bilan initial

**Sous-sections:**
1. **Informations du Bilan** : Données récupérées du bilan initial (objectif, niveau d'activité, antécédents)
2. **Suivi des Performances** : Projections & Profil, Évolution, Saisir une perf

#### 4. Hook de Persistance des Données

**Fichier:** `src/hooks/useFormPersistence.tsx`

**Fonctionnalités:**
- Sauvegarde automatique dans le localStorage
- Sauvegarde immédiate sur `visibilitychange` (changement de fenêtre/onglet)
- Sauvegarde avant fermeture de la page (`beforeunload`)
- Debounce de 500ms pendant la saisie
- Expiration configurable (24h par défaut, 7 jours pour les bilans)
- Notification de restauration avec options "Continuer" ou "Recommencer"
- Nettoyage automatique après soumission réussie

**Composants utilisant le hook:**
- `BilanSection` : Persistance des réponses du bilan (7 jours)
- `PerformanceEntry` : Persistance du formulaire de saisie (24h)
- `PerformanceSection` : Persistance de la sélection d'exercice (24h)

#### 5. Équipement dans les Listes Déroulantes

**Fichiers modifiés:**
- `ExerciseSelector.tsx` : Affichage `Nom (Equipement)`
- `ProjectionsDisplay.tsx` : Affichage `Nom (Equipement)`
- `PerformanceCharts.tsx` : Affichage `Nom (Equipement)`
- `WorkoutDatabase.tsx` : Affichage `Nom (Equipement)` dans la base de données
- `ExerciseListPage.tsx` : Affichage `Nom (Equipement)` côté client
- `WorkoutBuilder.tsx` : Affichage `Nom (Equipement)` dans le dropdown

### Difficultés Rencontrées

1. **Multiples itérations sur le design** : Le design initial avec grille de cartes ne correspondait pas aux attentes. Plusieurs aller-retours ont été nécessaires pour arriver à l'interface finale avec barre de recherche + vignette unique.

2. **Problème de persistance entre onglets** : L'état de sélection d'exercice était perdu lors du changement d'onglet. Résolu en remontant l'état au niveau du composant parent `PerformanceSection`.

3. **Graphiques vides malgré des données en base** : Le filtre utilisait `exerciseId` (camelCase) au lieu de `exercise_id` (snake_case) qui est le format retourné par Supabase.

4. **Calcul du 1RM incohérent** : Une performance de 50kg x 50 reps donnait un 1RM de 133kg, ce qui semblait incorrect mais était en fait correct selon la formule de Brzycki. Le problème venait des données de séance synchronisées qui contenaient des valeurs aberrantes.

5. **Fichier .ts vs .tsx** : Le hook `useFormPersistence` contenait du JSX mais avait l'extension `.ts`, causant une erreur de déploiement. Renommé en `.tsx`.
...
**Projection inverse:**
```
poids = 1RM × (1.0278 - 0.0278 × reps)
```

**Profil Nerveux:**
- Calculé en comparant les performances réelles aux projections théoriques
- Force : Meilleures performances sur les faibles répétitions (1-5)
- Endurance : Meilleures performances sur les hautes répétitions (10-15)
- Équilibré : Performances conformes aux projections

### Accès

- **Coach** : Accès complet via la fiche client (section "Objectif et Conditions d'Entraînement")
- **Client avec coach** : Accès via son profil (entre "Informations personnelles" et "Mes bilans")
- **Client sans coach** : Accès identique via son profil

---

## Intervention #1 - Système de Bilans Personnalisés et Récurrents (Décembre 2025)
...
## Intervention #2 - Implémentation des Profils Utilisateurs (Client & Coach)
...
## Intervention #3 - Restauration du Profil Client Côté Coach et Améliorations UX (Décembre 2025)
...
## Intervention #4 - Scanner de Code-Barres avec Open Food Facts
...
## Intervention #5 - Base de Données Alimentaire Ciqual & Filtres Avancés
...
## Intervention #6 - Drawer de Messagerie dans le Dashboard Coach
...
# RÉCAPITULATIF TECHNIQUE GLOBAL

Ce document a été restructuré pour présenter les interventions en ordre chronologique, suivi par un récapitulatif de l'architecture technique. Cette section synthétise les technologies, frameworks et méthodologies clés utilisés dans le projet Virtus, en se basant sur l'ensemble des interventions documentées.
...
| **Backend & Base de Données** | Supabase | Cloud (PostgreSQL) |
| **Hébergement** | Cloudflare Pages | - |
| **Tests Automatisés** | Vitest | 3.2.4 |
| **Gestion de Paquets** | pnpm | 10.11.1 |
| **Environnement Node.js** | Node.js | 22.16.0 |

## Architecture & Concepts Clés

### Frontend

- **State Management** : Le state management global est géré via `zustand` (implicite via `useDataStore.ts`), permettant un accès réactif aux données partagées comme la liste des aliments.
- **Routing** : La navigation est gérée par `react-router-dom` (implicite dans la structure des `pages`).
- **Composants UI** : L'interface est construite avec des composants réutilisables, incluant des modales (`createPortal`), des drawers, et des formulaires complexes.
- **Hooks personnalisés** : Des hooks comme `useUnreadCount` sont utilisés pour encapsuler et réutiliser la logique métier.

### Backend (Supabase)

- **Base de Données** : PostgreSQL est utilisé avec des politiques de **Row Level Security (RLS)** pour sécuriser l'accès aux données. Les vues SQL (`SECURITY INVOKER`) et les fonctions (`SET search_path`) sont configurées pour renforcer la sécurité.
- **Fonctions SQL** : Des fonctions atomiques (ex: `assign_bilan_atomic`) sont utilisées pour garantir l'intégrité des données lors d'opérations complexes.
- **Stockage** : Supabase Storage est utilisé pour stocker les fichiers uploadés (avatars, messages vocaux, documents) dans des buckets privés avec accès via URLs signées.
- **Realtime** : La fonctionnalité realtime de Supabase est utilisée pour la messagerie, permettant une mise à jour instantanée des conversations.

### Intégrations & APIs Externes

- **Open Food Facts** : Utilisée pour le scanner de code-barres afin de récupérer les informations nutritionnelles des produits.
- **Cloudinary** : Utilisé pour l'upload et la gestion des avatars utilisateurs.
- **Base de données Ciqual** : Intégrée pour enrichir la base de données alimentaire de l'application.

## Fonctionnalités Notables

- **Gestion de Bilans Personnalisés** : Système complet permettant aux coachs de créer, assigner et suivre des bilans récurrents pour leurs clients.
- **Scanner de Code-Barres** : Fonctionnalité mobile utilisant `html5-qrcode` et l'API Open Food Facts pour l'ajout rapide d'aliments.
- **Messagerie Intégrée** : Un système de messagerie complet avec support pour les messages texte, vocaux et le partage de documents, intégré directement dans le dashboard du coach via un drawer non-modal.
- **Base de Données Alimentaire Avancée** : Une base de données de plus de 3400 aliments (Ciqual) avec un système de filtres multi-niveaux et une recherche par mots-clés.
- **Profils Utilisateurs (Coach & Client)** : Gestion des profils avec upload d'avatar, fiches de présentation pour les coachs et paramètres de compte.

## DevOps & Qualité

- **CI/CD** : Le déploiement est automatisé via Cloudflare Pages, qui build et déploie l'application à chaque push sur la branche `main`. Des environnements de prévisualisation sont créés pour chaque Pull Request.
- **Tests Automatisés** : La logique métier critique, comme le calcul de progression des programmes et la gestion des bilans, est couverte par des tests unitaires avec Vitest.
