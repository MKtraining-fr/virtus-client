...
const ClientWorkoutBuilder: React.FC = () => {
  ...
  if (user && user.canUseWorkoutBuilder === false) {
    return (
      <div className="text-center py-10 bg-client-card rounded-lg">
        ...
        <p className="text-client-light mt-4">
          Vous n'avez pas la permission d'utiliser le créateur de séance.
        </p>
        <p className="text-client-subtle text-sm mt-2">
          Veuillez contacter votre coach pour plus d'informations.
        </p>
        <button
          onClick={() => navigate('/app/workout')}
          className="mt-6 bg-primary text-white font-semibold px-4 py-2 rounded-lg"
        >
  ...
  const availableExercises = useMemo(() => {
    return exerciseDB.filter(
      (ex) => ex.coachId === 'system' || ex.coachId === user?.coachId || !ex.coachId
    );
  }, [exerciseDB, user]);
          ...
          exercises: session.exercises.map((ex) => {
            ...
            if (field === 'sets') {
              ...
              if (newSets > currentSets) {
                const lastDetail = newDetails[currentSets - 1] || {
                  reps: '12',
                  load: { value: '', unit: 'kg' },
                  tempo: '2010',
                  rest: '60s',
  ...
  const addExercise = () => {
    ...
    const newExercise: WorkoutExercise = {
      ...
      details: Array(3)
        .fill(null)
        .map(() => ({ reps: '12', load: { value: '', unit: 'kg' }, tempo: '2010', rest: '60s' })),
      intensification: [],
      alternatives: [],
    };
    setSessions(
  ...
  const handleExerciseSelection = (
    selectedEx: Exercise,
    exerciseToUpdate: WorkoutExercise | null
  ) => {
    ...
    const hasExercises = sessions.some(
      (session) =>
        ...
        session.exercises.some(
          (ex) => ex.name.trim() !== '' && ex.name.trim() !== 'Choisir un exercice'
        )
    );
    if (!hasExercises) {
    ...
    <div className="flex flex-col h-[calc(100vh-128px)] text-gray-800 dark:text-client-light">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-4">
          ...
          <ClientAccordion
            title={workoutMode === 'program' ? 'Détails du Programme' : 'Détails de la Séance'}
          >
            {workoutMode === 'program' ? (
          ...
          {workoutMode === 'program' && (
            <div className="flex items-center gap-2 pb-2 overflow-x-auto">
              ...
              <button
                onClick={addSession}
                className="w-9 h-9 bg-gray-100 dark:bg-client-card rounded-full flex items-center justify-center text-gray-600 dark:text-client-subtle hover:bg-primary hover:text-white transition-colors flex-shrink-0"
              >
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          <label
            ...
            className="block text-xs text-gray-500 dark:text-client-subtle mb-1 text-center"
          >
            Séries
          </label>
          <Input
        ...
        <div>
          ...
          <div className="flex">
            <Input
              id={`${idPrefix}-load-value`}
              type="number"
              value={exercise.details[0]?.load.value || ''}
              onChange={(e) => onUpdate(exercise.id, 'load.value', e.target.value)}
              className="text-center rounded-r-none h-[42px]"
            />
            <Select
              value={exercise.details[0]?.load.unit || 'kg'}
              onChange={(e) => onUpdate(exercise.id, 'load.unit', e.target.value)}
              className="rounded-l-none -ml-px !py-0 h-[42px]"
            >
              <option>kg</option>
              <option>%</option>
              <option>RPE</option>
      ...
      {isExpanded && (
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200 dark:border-client-dark/50">
          <div>
            ...
            <Input
              id={`${idPrefix}-tempo`}
              value={exercise.details[0]?.tempo || ''}
          ...
          <div className="col-span-2">
            <label
              htmlFor={`${idPrefix}-intensification`}
              className="block text-xs text-gray-500 dark:text-client-subtle mb-1 text-left"
            >
              Intensification
            </label>
            <Select
              id={`${idPrefix}-intensification`}
              value={exercise.intensification?.[0]?.value || 'Aucune'}
              onChange={(e) => onUpdate(exercise.id, 'intensification', e.target.value)}
              className="w-full"
            >
...
const ExerciseSelectionModal: React.FC<ExerciseSelectionModalProps> = ({
  ...
  const equipmentTypes = useMemo(
    () => Array.from(new Set(availableExercises.map((e) => e.equipment).filter(Boolean))),
    [availableExercises]
  ) as string[];
  const muscleGroups = useMemo(
    () => Array.from(new Set(availableExercises.flatMap((e) => e.muscleGroups).filter(Boolean))),
    [availableExercises]
  ) as string[];
          ...
          {filteredExercises.length > 0 ? (
            filteredExercises.map((ex) => (
              <button
                key={ex.id}
                onClick={() => onSelect(ex)}
                className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-client-dark text-left transition-colors"
              >
                <img
