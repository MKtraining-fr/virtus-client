        ...
        if (isEditMode && editProgramId) {
            ...
            if (workoutMode === 'program') {
                ...
                if (selectedClient && selectedClient !== '0') {
                    const updatedClients = clients.map(c => {
                        if (c.id === selectedClient) {
                            const hasCurrentProgram = c.assignedPrograms && c.assignedPrograms.length > 0;
                            if (hasCurrentProgram) {
                                // Add as next program, don't touch progress
                                const updatedPrograms = [...c.assignedPrograms, newProgram];
                                return { ...c, assignedPrograms: updatedPrograms, viewed: false };
                            } else {
                                // Set as current program and initialize progress
                                ...
                                return { ...c, assignedPrograms: [newProgram], ...progressUpdates };
                            }
                        }
                        return c;
                    });
                    ...
                    addNotification({
                     ...
                     alert(`Programme "${programName}" enregistré dans la bibliothèque.`);
                }
