...
const ProFormationManagement: React.FC = () => {
  ...
  const [currentFormation, setCurrentFormation] = useState<Partial<ProfessionalFormation> | null>(
    null
  );

  const openModal = (
    mode: 'add' | 'edit',
    formation: Partial<ProfessionalFormation> | null = null
  ) => {
    setModalMode(mode);
    ...
    <div>
              ...
              {professionalFormations.map((formation) => (
                <tr key={formation.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap space-x-2">
                    ...
                    <Button size="sm" variant="danger" onClick={() => handleDelete(formation.id)}>
                      Supprimer
                    </Button>
                  </td>
                </tr>
      ...
      {isModalOpen && currentFormation && (
        <Modal
          isOpen={isModalOpen}
          onClose={closeModal}
          title={modalMode === 'add' ? 'Ajouter une Formation' : 'Modifier la Formation'}
        >
          <div className="space-y-4">
            <Input
              ...
              onChange={(e) =>
                setCurrentFormation((p) => (p ? { ...p, title: e.target.value } : null))
              }
            />
            <div>
            ...
            <Input
              ...
              value={currentFormation.coverImageUrl}
              onChange={(e) =>
                setCurrentFormation((p) => (p ? { ...p, coverImageUrl: e.target.value } : null))
              }
            />
            <Input
              label="Prix (€)"
            ...
            <Select
              label="Type d'accès"
              value={currentFormation.accessType}
              onChange={(e) =>
                setCurrentFormation((p) =>
                  p ? { ...p, accessType: e.target.value as 'purchase' | 'subscription' } : null
                )
              }
            >
