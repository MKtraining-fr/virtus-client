...
const ExerciseListPage: React.FC<ExerciseListPageProps> = ({ title, exercises, children }) => {
    ...
    const equipmentTypes = useMemo(() => Array.from(new Set(exercises.map(e => e.equipment).filter(Boolean))), [exercises]) as string[];
    const muscleGroups = useMemo(() => Array.from(new Set(exercises.flatMap(e => e.muscleGroups).filter(Boolean))), [exercises]) as string[];
            ...
            <ClientAccordion title="Filtres">
                 <div className="space-y-6">
                    ...
                    {equipmentTypes.length > 0 && (
                        <div>
                            <h3 className="font-semibold mb-3 text-gray-900 dark:text-client-light">Équipement</h3>
                    ...
                    {muscleGroups.length > 0 && (
                        <div>
                            <h3 className="font-semibold mb-3 text-gray-900 dark:text-client-light">Groupes musculaires</h3>
                            <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex justify-end pt-4">
                        <Button variant="secondary" onClick={handleResetFilters}>Réinitialiser</Button>
                    </div>
                </div>
            </ClientAccordion>
                ...
                {filteredExercises.map(exercise => (
                    ...
                    >
                        ...
                        <div className="flex-grow">
                            ...
                            <p className="text-xs text-gray-500 dark:text-client-subtle">{exercise.muscleGroups?.join(', ')}</p>
                        </div>
                    </div>
                ))}
                 {filteredExercises.length === 0 && (
                    <p className="text-center text-gray-500 dark:text-client-subtle py-8">Aucun exercice ne correspond à votre recherche.</p>
                )}
            </div>
