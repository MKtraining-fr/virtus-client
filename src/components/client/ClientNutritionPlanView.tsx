      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-client-dark">
            <tr>
              <th className="p-2 text-left font-semibold text-client-subtle sticky left-0 bg-client-dark z-10">
                Jour
              </th>
              {mealNames.map((name) => (
                <th
                  key={name}
                  className="p-2 text-left font-semibold text-client-subtle min-w-[220px]"
                >
                  {name}
                </th>
              ))}
              <th className="p-2 text-left font-semibold text-client-subtle min-w-[150px]">
                Totaux Journaliers
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-700">
            {weekDays.map((day) => {
              ...
              return (
                <tr key={day.id}>
                  <td className="p-2 font-bold align-top sticky left-0 bg-client-card">
                    {day.name}
                  </td>
                  {mealNames.map((mealName) => {
                    const meal = day.meals.find((m) => m.name === mealName);
                    const mealMacros = meal
                      ? calculateMacros(meal.items)
                      : { calories: 0, protein: 0, carbs: 0, fat: 0 };
                          ...
                          <div className="space-y-1">
                            {meal.items.map((item) => (
                              <div key={item.id} className="flex justify-between text-client-light">
                                ...
                                <span className="font-medium text-client-light whitespace-nowrap">
                                  {item.quantity}
                                  {item.unit}
                                </span>
                              </div>
                            ...
                            <div className="pt-2 mt-2 border-t border-gray-700 text-xs text-client-subtle font-semibold space-y-0.5">
                              ...
                              <div className="flex justify-between flex-wrap gap-x-2">
                                ...
                                <span className="text-green-400">
                                  G: {Math.round(mealMacros.carbs)}g
                                </span>
                                <span className="text-yellow-400">
                                  L: {Math.round(mealMacros.fat)}g
                                </span>
                              </div>
                            </div>
                          </div>
        ...
        {weekDays.length === 0 && (
          <p className="text-center text-client-subtle py-4">
