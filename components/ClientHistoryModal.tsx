...
import { Client, WorkoutProgram } from '../types.ts';
import ProgramDetailView from './ProgramDetailView.tsx';
...
const ArrowUpRightOnBoxIcon = (props: React.SVGProps<SVGSVGElement>) => ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9" /></svg>);
...
const ClientHistoryModal: React.FC<ClientHistoryModalProps> = ({ clientId, isOpen, isMinimized, onMinimize, onRestore, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-40 flex justify-center items-center" onClick={onClose}>
            <div className="bg-light-bg rounded-lg shadow-2xl w-full h-full max-w-7xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center p-4 border-b bg-white rounded-t-lg">
                    <h2 className="text-xl font-bold">Historique de {client?.firstName} {client?.lastName}</h2>
                        ...
                        <ul className="space-y-2">
                            {client?.assignedPrograms?.map(program => (
                                <li key={program.id}>
                                    <label className="flex items-center p-2 rounded-md hover:bg-gray-100 cursor-pointer">
                                        <input
                                            ...
                                            disabled={!selectedProgramIds.includes(program.id) && selectedProgramIds.length >= 3}
                                        />
                             ...
                             {(!client?.assignedPrograms || client.assignedPrograms.length === 0) && (
                                <p className="text-sm text-gray-500 text-center">Aucun programme dans l'historique.</p>
                             )}
                        </ul>
                    </div>
