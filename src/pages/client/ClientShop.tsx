...
const PartnerCard: React.FC<{ partner: Partner }> = ({ partner }) => (
    <div className="bg-client-card rounded-lg p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 border border-gray-700">
        <img src={partner.logoUrl} alt={partner.name} className="w-20 h-20 sm:w-24 sm:h-24 rounded-lg object-contain bg-white p-1 flex-shrink-0" />
...
const ShopContent: React.FC<{ products: Product[], partners: Partner[] }> = ({ products, partners }) => {
...
const ClientShop: React.FC = () => {
    ...
    const coachProducts = useMemo(() => products.filter(p => p.ownerId === user?.coachId), [products, user]);
    const coachPartners = useMemo(() => partners.filter(p => p.ownerId === user?.coachId), [partners, user]);
    ...
    return (
        <div className="space-y-6">
            ...
            {hasAdminAccess && activeTab === 'admin' && <ShopContent products={adminProducts} partners={adminPartners} />}
            {hasCoachAccess && activeTab === 'coach' && <ShopContent products={coachProducts} partners={coachPartners} />}
