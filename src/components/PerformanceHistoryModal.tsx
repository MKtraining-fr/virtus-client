...
const PerformanceHistoryModal: React.FC<PerformanceHistoryModalProps> = ({ isOpen, onClose, clientId }) => {
    ...
    const programToShowData = useMemo(() => {
        ...
        const sortedLogs = [...client.performanceLog].sort((a, b) => 
    ...
    const modalTitle = programToShowData ? `Historique de performance pour : ${programToShowData.program.name}` : "Historique de performance";

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={modalTitle} size="xl">
            {programToShowData ? (
                <ProgramPerformanceDetail program={programToShowData.program} performanceLogs={programToShowData.logs} />
            ) : (
                <div className="text-center py-10">
                    <p className="text-gray-500">Aucun historique de performance à afficher pour ce client.</p>
                </div>
            )}
        </Modal>
    );
