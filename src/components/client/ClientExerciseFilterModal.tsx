...
const ClientExerciseFilterModal: React.FC<ClientExerciseFilterModalProps> = ({
  isOpen,
  onClose,
  allExercises,
  activeFilters,
  ...
  const equipmentTypes = useMemo(
    () => Array.from(new Set(allExercises.map((e) => e.equipment).filter(Boolean))),
    [allExercises]
  ) as string[];
  const muscleGroups = useMemo(
    () => Array.from(new Set(allExercises.flatMap((e) => e.muscleGroups).filter(Boolean))),
    [allExercises]
  ) as string[];
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Filtres" theme={theme}>
      <div className="space-y-6">
        ...
        {muscleGroups.length > 0 && (
          <div>
            <h3 className="font-semibold mb-2 text-gray-800 dark:text-client-light">
              Groupes musculaires
            </h3>
            <div className="flex flex-wrap gap-2">
