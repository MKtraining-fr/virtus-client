        ...
        <div className="space-y-4">
            <div className="flex items-center gap-4">
                <button onClick={() => navigate(-1)} className="p-2 bg-white dark:bg-client-card rounded-full text-gray-800 dark:text-client-light hover:bg-gray-100 dark:hover:bg-primary/20 border border-gray-300 dark:border-gray-700" aria-label="Retour">
                 ...
                 <h2 className="text-xl font-bold text-gray-900 dark:text-client-light">Base de données Ciqual</h2>
            </div>
            ...
            <div className="max-h-[50vh] overflow-y-auto space-y-1 pr-2">
                {filteredFood.slice(0, 200).map(food => (
                    <div key={food.name} className="p-3 bg-white dark:bg-client-card rounded-lg border border-gray-200 dark:border-transparent">
                        <p className="font-semibold text-gray-900 dark:text-client-light">{food.name}</p>
                        <p className="text-xs text-gray-500 dark:text-client-subtle">{food.calories} kcal | P:{food.protein}g | G:{food.carbs}g | L:{food.fat}g (pour 100g)</p>
                    </div>
                ))}
                {filteredFood.length === 0 && (
                    <p className="text-center text-gray-500 dark:text-client-subtle py-8">Aucun aliment ne correspond à votre recherche.</p>
                )}
            </div>
        </div>
    );
