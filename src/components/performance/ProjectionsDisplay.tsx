...
export const ProjectionsDisplay: React.FC<ProjectionsDisplayProps> = ({ clientId }) => {
  ...
  const getUniqueExercises = (): ExerciseOption[] => {
    const exercises = new Map<string, string>();
    projections.forEach(p => {
      if (!exercises.has(p.exerciseId)) {
        exercises.set(p.exerciseId, p.exerciseName || 'Exercice inconnu');
      }
    });
  ...
  const selectedExercise = uniqueExercises.find(ex => ex.id === selectedExerciseId);
  ...
  return (
    <div className="space-y-6">
      {/* Barre de recherche ou Vignette sélectionnée */}
      {!selectedExerciseId ? (
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 px-4 py-3 bg-primary text-white rounded-xl flex items-center justify-between">
      ...
      {/* Contenu des projections (affiché seulement si un exercice est sélectionné) */}
      {selectedExerciseId && (
