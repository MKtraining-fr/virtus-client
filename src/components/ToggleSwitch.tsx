...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ label1, value1, label2, value2, value, onChange, theme = 'light' }) => {
  ...
  return (
    <div className={`flex items-center space-x-4 p-1 ${containerClasses} rounded-full`}>
        ...
        >
            <div className={`bg-white w-6 h-6 rounded-full shadow-md transform transition-transform ${!isValue1Active ? 'translate-x-8' : ''}`}></div>
        </div>
        <span 
            className={`font-semibold text-lg cursor-pointer transition-colors px-4 ${!isValue1Active ? 'text-primary' : inactiveLabelClasses}`}
