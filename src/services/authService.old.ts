/**
 * Service d'authentification utilisant Firebase Authentication
 * Ce service remplace l'ancien système d'authentification custom avec mots de passe en clair
 */
...
export const signUp = async (data: SignUpData): Promise<Client> => {
  try {
    ...
    let affiliationCode = data.affiliationCode;
    if (data.role === 'coach' && !affiliationCode) {
