...
import { PerformanceLog, PerformanceSet, WorkoutProgram, WorkoutExercise, Exercise, ExerciseLog, Client } from '../../../types';
import Modal from '../../../components/Modal';
import Button from '../../../components/Button';
import SessionRecapModal from '../../../components/client/SessionRecapModal';
import {
    ArrowLeftIcon, ClockIcon, PencilIcon, ChevronDownIcon, EllipsisVerticalIcon, PlayCircleIcon,
    ArrowsRightLeftIcon, ChatBubbleLeftIcon, XMarkIcon, PlayIcon, PauseIcon, TrophyIcon
} from '../../../constants/icons';


const INTENSIFICATION_DEFINITIONS: Record<string, string> = {
    'Drop Set': 'Technique consistant à effectuer une série jusqu\'à l\'échec, puis à réduire immédiatement le poids et à continuer avec plus de répétitions jusqu\'à l\'échec à nouveau.',
};
...
const ClientCurrentProgram: React.FC = () => {
    ...
    const [recapData, setRecapData] = useState<{ exerciseLogs: ExerciseLog[], sessionName: string } | null>(null);
    
    const [isCommentModalOpen, setIsCommentModalOpen] = useState(false);
    const [commentTarget, setCommentTarget] = useState<{ exerciseId: number; setIndex: number } | null>(null);
    ...
    const defaultSessionIndex = useMemo(() => {
        ...
        const sessionsForCurrentWeek = baseProgram.sessionsByWeek[currentWeek] || baseProgram.sessionsByWeek[1];
    ...
    const availableSessions = useMemo(() => {
        ...
        const sessionsForWeek = localProgram.sessionsByWeek[currentWeek] || localProgram.sessionsByWeek[1] || [];
    ...
    const activeSession = useMemo(() => {
        ...
        const sessionsForWeek = localProgram.sessionsByWeek[currentWeek] || localProgram.sessionsByWeek[1]; // Fallback to week 1
    ...
    useEffect(() => {
        if (activeSession) {
            ...
            for (const exercise of activeSession.exercises) {
                ...
                initialLogData[exercise.id.toString()] = Array.from({ length: totalSets }, (): PerformanceSet => ({ reps: '', load: '' }));
            }
    ...
    const previousPerformancePlaceholders = useMemo(() => {
        ...
        const lastSessionLog = user.performanceLog
            .slice()
            .filter(log => log.programName === localProgram?.name && log.sessionName === activeSession.name)
            .pop();
    ...
    const currentExercise = useMemo(() => {
        if (!activeSession || !activeSession.exercises || activeSession.exercises.length <= currentExerciseIndex) return null;
    ...
    const handleLogChange = (exerciseId: number, setIndex: number, field: 'reps' | 'load', value: string) => {
        ...
        setLogData(prev => {
            ...
            const existingLog = prev[exerciseKey] || Array.from({ length: currentExerciseSets }, (): PerformanceSet => ({ reps: '', load: '' }));
    ...
    const handleFinishSession = () => {
        ...
        if (hasUnloggedExercises) {
            if (!window.confirm("Certains exercices ne sont pas complétés. Voulez-vous vraiment terminer la séance ? Les données non saisies ne seront pas enregistrées.")) {
                return;
            }
        }
        ...
        const exerciseLogsForSession: ExerciseLog[] = activeSession.exercises
            .map(exercise => {
                const loggedSetsForExercise = logData[exercise.id.toString()] || [];
                const nonEmptySets = loggedSetsForExercise.filter(s => s.reps.trim() || s.load.trim() || s.comment?.trim());
        ...
        const updatedClients = clients.map(c => {
            if (c.id === user.id) {
                ...
                const sessionsForCurrentWeek = localProgram.sessionsByWeek[currentProgramWeek] || localProgram.sessionsByWeek[1] || [];
                ...
                const isLastSessionOfProgram = currentProgramWeek >= totalWeeks && currentSessionProgress >= totalSessionsForCurrentWeek;
                ...
                if (isLastSessionOfProgram) {
                    ...
                    if (nextSessionProgress > totalSessionsForCurrentWeek) {
                        ...
                        const sessionsForNextWeek = localProgram.sessionsByWeek[nextProgramWeek] || localProgram.sessionsByWeek[1] || [];
    ...
    const TimerDisplay = () => {
        ...
        const timeColorClass = elapsedTime > restTimeInSeconds ? 'text-red-500' : 'text-gray-900 dark:text-client-light';
    
        return (
            <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4">
                <button onClick={handleCloseTimer} className="absolute top-6 right-6 text-gray-400 dark:text-client-subtle hover:text-white" aria-label="Fermer le chronomètre">
                    <XMarkIcon className="w-8 h-8" />
                </button>
                <p className="text-xl text-gray-400 dark:text-client-subtle mb-4">Objectif: {restTimeInSeconds === Infinity ? '-' : `${restTimeInSeconds}s`}</p>
                <div className={`font-mono font-bold text-8xl md:text-9xl ${timeColorClass}`}>
                    {formatTime(elapsedTime)}
                </div>
                <div className="mt-12 flex items-center gap-8">
                    <Button variant="secondary" onClick={handleResetTimer} className="!font-semibold">Réinitialiser</Button>
                    <button onClick={toggleTimer} className="w-24 h-24 bg-white text-client-dark rounded-full flex items-center justify-center text-2xl font-bold shadow-lg transform active:scale-95 transition-transform" aria-label={isTimerRunning ? "Mettre en pause" : "Démarrer"}>
                        {isTimerRunning ? <PauseIcon className="w-10 h-10" /> : <PlayIcon className="w-10 h-10 ml-1" />}
                    </button>
                </div>
            </div>
        );
    ...
    if (!localProgram || !activeSession) {
        return (
            <div className="text-center py-10">
                <p className="text-gray-900 dark:text-client-light text-lg">Aucun programme ou séance active.</p>
                <p className="text-gray-500 dark:text-client-subtle mt-1">Contactez votre coach.</p>
                <button onClick={() => navigate('/app/workout')} className="mt-6 text-primary hover:underline">
                    Retour
                </button>
            </div>
        );
    ...
    if (!currentExercise) {
         return (
            <div className="text-center py-10">
                <p className="text-gray-900 dark:text-client-light text-lg">Cette séance ne contient aucun exercice.</p>
            </div>
         )
    }
    ...
    return (
        <div className="relative pb-20">
            ...
            <div className="flex items-center gap-4 mb-4">
                <button onClick={() => navigate(-1)} className="p-2 bg-white dark:bg-client-card rounded-full text-gray-800 dark:text-client-light hover:bg-gray-100 dark:hover:bg-primary/20 flex-shrink-0 border border-gray-300 dark:border-gray-700" aria-label="Retour">
            ...
            <div className="bg-white dark:bg-client-card rounded-lg p-3 space-y-4 shadow-sm border border-gray-200 dark:border-transparent">
                <div className="flex justify-between items-center gap-2">
                    ...
                    <div className="relative">
                        <button ref={optionsButtonRef} onClick={() => setIsOptionsPopoverOpen(prev => !prev)} className="w-10 h-10 bg-gray-100 dark:bg-client-dark rounded-full flex items-center justify-center text-gray-500 dark:text-client-subtle hover:text-primary transition-colors flex-shrink-0">
                        ...
                        {isOptionsPopoverOpen && (
                             <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-client-dark rounded-lg shadow-lg p-2 z-30 border border-gray-200 dark:border-gray-700">
                                <button onClick={() => { setIsYouTubeModalOpen(true); setIsOptionsPopoverOpen(false); }} className="w-full flex items-center gap-3 p-2 text-left rounded-md text-gray-800 dark:text-client-light hover:bg-gray-100 dark:hover:bg-primary/20">
                                    <PlayCircleIcon className="w-5 h-5"/>
                                    <span>Voir la vidéo</span>
                                </button>
                                <button onClick={() => { setIsAlternativesModalOpen(true); setIsOptionsPopoverOpen(false); }} className="w-full flex items-center gap-3 p-2 text-left rounded-md text-gray-800 dark:text-client-light hover:bg-gray-100 dark:hover:bg-primary/20">
                ...
                <div className="grid grid-cols-4 divide-x divide-gray-200 dark:divide-client-dark/50 text-center">
                    <div><p className="text-sm text-gray-500 dark:text-client-subtle">Séries</p><p className="font-bold text-lg">{currentExercise.sets}</p></div>
                    <div><p className="text-sm text-gray-500 dark:text-client-subtle">Rép</p><p className="font-bold text-lg">{getDisplayValue(currentExercise.details, 'reps')}</p></div>
                    <div><p className="text-sm text-gray-500 dark:text-client-subtle">Repos</p><p className="font-bold text-lg">{getDisplayValue(currentExercise.details, 'rest').replace(/\D/g, '')}</p></div>
                    <div><p className="text-sm text-gray-500 dark:text-client-subtle">Tempo</p><p className="font-bold text-lg">{getDisplayValue(currentExercise.details, 'tempo')}</p></div>
                </div>
                ...
                {technique && definition && (
                    <div className="relative">
                        ...
                        >
                            <span>Technique d'intensification : <strong>{technique}</strong></span>
                            <ChevronDownIcon className={`w-4 h-4 text-gray-500 dark:text-client-subtle transition-transform ${isDefinitionVisible ? 'rotate-180' : ''}`} />
                        </button>
                        {isDefinitionVisible && (
                                ...
                                >
                                    ...
                                    <div className="flex-1 px-1">
                                        <input
                                            ...
                                            value={repValue}
                                            onChange={e => handleLogChange(currentExercise.id, setIndex, 'reps', e.target.value)}
                                    ...
                                    <div className="flex-1 px-1">
                                        <input
                                            ...
                                            value={loadValue}
                                            onChange={e => handleLogChange(currentExercise.id, setIndex, 'load', e.target.value)}
                                            onFocus={() => setActiveSetIndex(setIndex)}
                                            className={`w-full rounded-md text-center py-2 font-bold text-lg border-2 ${isSetSelected ? 'bg-white/20 border-white/50 text-white placeholder:text-white/70' : 'bg-white dark:bg-client-card border-gray-300 dark:border-transparent text-gray-900 dark:text-client-light placeholder:text-gray-500 dark:placeholder:text-client-subtle'}`}
                                            onClick={(e) => e.stopPropagation()}
                                        />
                                    ...
                                    <div className="flex-none w-10 text-center pl-1 shrink-0">
                                        {isSetSelected ? (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleOpenCommentModal(currentExercise.id, setIndex); }}
                                            ...
                                            logData[exerciseKey]?.[setIndex]?.comment && (
                                                <ChatBubbleLeftIcon className="w-5 h-5 text-gray-500 dark:text-client-subtle mx-auto" title={logData[exerciseKey]?.[setIndex]?.comment} />
                                            )
                                        )}
                                    </div>
                                </div>
            ...
            <div className="mt-8">
                 <button onClick={handleFinishSession} className="w-full bg-primary text-white font-bold py-3 rounded-lg hover:bg-violet-700 transition-colors">
                    Terminer la séance
                </button>
            </div>
            ...
            >
                <div className="text-center">
                    ...
                    <button onClick={handleCloseCongratsModal} className="mt-6 bg-primary text-white font-bold py-2 px-6 rounded-lg">
                ...
                <div className="space-y-2">
                    {alternativeExercises.length > 0 ? (
                        alternativeExercises.map(alt => (
                             <button key={alt.id} onClick={() => handleSwapExercise(alt)} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 text-left transition-colors">
                                <img src={alt.illustrationUrl} alt={alt.name} className="w-16 h-16 object-contain rounded-md bg-gray-100 flex-shrink-0" />
