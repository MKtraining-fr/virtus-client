...
const UnviewedDot = () => (
  <span className="w-2 h-2 bg-red-500 rounded-full shrink-0" title="Non visualisé"></span>
);
  ...
  return (
    <React.Fragment>
      ...
      <tr className={`${rowBgClass} ${hoverBgClass}`}>
        <td
          className={`sticky left-0 ${rowBgClass} p-2 font-medium text-black border-r w-48 min-w-[192px]`}
        >
          <div className="flex items-center">
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className="p-1 text-gray-500 hover:text-primary mr-2"
            >
              <ChevronDownIcon
        ...
        {[...Array(weekCount)].map((_, weekIndex) => {
          ...
          const prevExerciseLogInLog = prevLog?.exerciseLogs.find(
          ...
          const hasUnviewedSets = exerciseLogInLog?.loggedSets.some(
            (s) => s.viewedByCoach === false
          );
      ...
      {isExpanded &&
        [...Array(totalSets)].map((_, setIndex) => (
          <tr key={setIndex} className={`${rowBgClass} text-sm`}>
            <td
              className={`sticky left-0 ${rowBgClass} p-2 pl-12 text-gray-700 italic border-r w-48 min-w-[192px]`}
            >
              — Série {setIndex + 1}
            </td>
            {[...Array(weekCount)].map((_, weekIndex) => {
              const week = weekIndex + 1;
              const set = logsByWeekAndSession
                .get(week)
                ?.get(sessionName)
                ?.exerciseLogs.find((e) => e.exerciseName === exercise.name)?.loggedSets[setIndex];
              const prevSet = logsByWeekAndSession
                .get(week - 1)
                ?.get(sessionName)
                ?.exerciseLogs.find((e) => e.exerciseName === exercise.name)?.loggedSets[setIndex];

              const repsClass = getComparisonClass(
                set ? parseFloat(set.reps) : null,
                prevSet ? parseFloat(prevSet.reps) : null
              );
              const loadClass = getComparisonClass(
                set ? parseFloat(set.load) : null,
...
const ProgramPerformanceDetail: React.FC<ProgramPerformanceDetailProps> = ({
  program,
  performanceLogs,
}) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <label htmlFor="session-filter" className="font-semibold">
          Filtrer par séance:
        </label>
        <select
          id="session-filter"
      ...
      <div className="overflow-x-auto border rounded-lg max-h-[65vh]">
        <table className="w-full text-sm table-auto border-collapse">
          <thead className="sticky top-0 bg-gray-200 z-10">
            <tr>
              <th className="sticky left-0 bg-gray-200 p-2 text-left font-semibold border-r w-48 min-w-[192px] text-black">
                Exercice
              </th>
              {[...Array(program.weekCount)].map((_, i) => (
                <th
                  key={i}
                  className="p-2 text-left font-semibold border-l min-w-[192px] text-black"
                >
                  Semaine {i + 1}
                </th>
              ))}
              ...
              <React.Fragment key={session.name}>
                {selectedSessionName === 'all' && (
                  <tr className="bg-gray-100">
                    <td
                      colSpan={program.weekCount + 1}
                      className="p-2 font-bold text-black sticky left-0 bg-gray-100 border-r"
                    >
                      {session.name}
                    </td>
                  </tr>
        ...
        {filteredSessions.length === 0 && (
          <div className="p-6 text-center text-gray-500">Aucun exercice dans cette séance.</div>
        )}
      </div>
    </div>
  );
