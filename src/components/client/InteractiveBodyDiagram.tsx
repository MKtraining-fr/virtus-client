...
const MUSCLE_AREAS: { group: string; x: number; y: number; width: number; height: number; side: 'front' | 'back' }[] = [
...
const InteractiveBodyDiagram: React.FC<InteractiveBodyDiagramProps> = ({ selectedGroups, onToggleGroup }) => {
        ...
        <div>
            <div className="relative mx-auto" style={{ width: 200, height: 300 }}>
                <svg viewBox="0 0 200 300" className="w-full h-full">
                    {/* Basic Silhouette */}
                    <path d="M100 10 C 110 10 110 20 100 20 C 90 20 90 10 100 10 Z" className="fill-gray-200 dark:fill-gray-700" /> {/* Head */}
                    <path d="M90 20 L110 20 L115 50 L85 50 Z" className="fill-gray-200 dark:fill-gray-700" /> {/* Neck */}
                    <path d="M80 50 L120 50 L140 100 L120 140 L80 140 L60 100 Z" className="fill-gray-200 dark:fill-gray-700" /> {/* Torso */}
                    <path d="M100 140 L110 220 L100 280 L90 220 Z" className="fill-gray-200 dark:fill-gray-700" /> {/* Legs */}
                </svg>
            ...
            <div className="flex justify-center gap-4 mt-2">
                <button onClick={() => setView('front')} className={`px-3 py-1 rounded-full text-sm ${view === 'front' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-client-card text-gray-800 dark:text-client-light'}`}>Avant</button>
                <button onClick={() => setView('back')} className={`px-3 py-1 rounded-full text-sm ${view === 'back' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-client-card text-gray-800 dark:text-client-light'}`}>Arrière</button>
            </div>
        </div>
    );
};
