...
const ProgramCard: React.FC<{ program: WorkoutProgram; isNext?: boolean; onView: () => void }> = ({
  program,
  isNext = false,
  onView,
}) => (
    ...
    <div className="mt-4">
      <div className="flex justify-between items-center mt-4 text-sm">
        <span className="text-gray-800 dark:text-client-light">
          {program.sessionsByWeek[1]?.length || 0} séance(s)
        </span>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {savedByClient.map((program) => (
              <ProgramCard key={program.id} program={program} onView={() => handleView(program)} />
            ))}
          </div>
        </div>
      )}
