...
const BilanArchive: React.FC = () => {
    ...
    const filteredAndSortedArchives = useMemo(() => {
        let sortableArchives = [...archives];

        if (filter) {
            sortableArchives = sortableArchives.filter(archive =>
        ...
        return sortableArchives;
    }, [archives, filter, sortConfig]);

    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedArchives(filteredAndSortedArchives.map(a => a.id));
        } else {
            setSelectedArchives([]);
        }
    };
                            ...
                            <tr>
                                <th className="p-4">
                                    <input 
                                        ...
                                        checked={filteredAndSortedArchives.length > 0 && selectedArchives.length === filteredAndSortedArchives.length}
                                        disabled={filteredAndSortedArchives.length === 0}
                                    />
                                </th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredAndSortedArchives.map(archive => (
                                <tr key={archive.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleBilanClick(archive)}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{archive.registrationDate}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
