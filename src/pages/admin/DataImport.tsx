...
const IMPORT_CONFIG: { [key in ImportableKey]: { title: string; requiredHeaders: string[]; description: string } } = {
    users: { 
        title: 'Utilisateurs (Coachs & Clients)', 
        requiredHeaders: ['firstName', 'lastName', 'email'],
        description: 'Importer des utilisateurs. Les utilisateurs avec status=active auront un compte Auth créé automatiquement.'
    },
    exercises: { 
    ...
    products: { 
        ...
        requiredHeaders: ['name', 'description', 'price', 'category', 'imageUrl', 'productUrl', 'ownerId'],
...
const DataImport: React.FC = () => {
    ...
    const downloadTemplate = (key: ImportableKey) => {
        ...
        switch (key) {
            case 'users':
                exampleRows = 'Jean,Dupont,jean.dupont@example.com,0612345678,client,active\nMarie,Martin,marie.martin@example.com,0687654321,client,prospect';
                break;
            case 'exercises':
                exampleRows = 'Développé couché,Musculation,Exercice pour les pectoraux,Barre|Banc,Pectoraux|Triceps|Épaules\nSquat,Musculation,Exercice pour les jambes,Barre,Quadriceps|Fessiers';
                break;
            case 'ciqual':
        ...
        <div className="p-6">
            <div className="mb-8">
                ...
                <p className="text-gray-600 mb-4">
                    Utilisez cette page pour importer en masse des données dans l'application. 
