...
const ClientWorkout: React.FC = () => {
    ...
    const totalSessions = (program?.sessionsByWeek?.[currentWeek] || program?.sessionsByWeek?.[1] || []).length || 1;

    const ActionButton: React.FC<{title: string, subtitle: string, to: string, disabled?: boolean}> = ({title, subtitle, to, disabled}) => (
        <Link 
            to={disabled ? '#' : to} 
        ...
        >
            ...
            {!disabled && <ChevronRightIcon className="w-6 h-6 text-gray-400 dark:text-client-subtle transition-transform group-hover:translate-x-1 group-hover:text-primary" />}
        </Link>
    );
    ...
    return (
        <div className="space-y-6">
            {hasAssignedProgram && program ? (
                <Link to="/app/workout/current-program" className="block bg-white dark:bg-client-card rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-primary/10 transition-colors group border border-gray-200 dark:border-transparent">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="font-bold text-gray-800 dark:text-client-light text-lg">Programme en cours</h3>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-4 text-center bg-gray-100 dark:bg-client-dark p-3 rounded-lg">
                        <div>
                            ...
                            <p className="text-4xl font-bold text-gray-900 dark:text-client-light">{currentWeek}<span className="text-xl text-gray-400 dark:text-client-subtle">/{totalWeeks}</span></p>
                        </div>
                         <div>
                            <p className="text-sm text-gray-500 dark:text-client-subtle">Séance</p>
                            <p className="text-4xl font-bold text-gray-900 dark:text-client-light">{currentSession}<span className="text-xl text-gray-400 dark:text-client-subtle">/{totalSessions}</span></p>
                        </div>
                    </div>
                </Link>
            ) : (
                 <div className="bg-white dark:bg-client-card rounded-lg p-6 text-center border border-gray-200 dark:border-transparent">
                    <h3 className="font-bold text-gray-800 dark:text-client-light text-lg">Programme en cours</h3>
                    <p className="text-sm text-gray-500 dark:text-client-subtle mt-2">Aucun programme ne vous a été assigné pour le moment. Contactez votre coach pour commencer !</p>
                </div>
            )}
