...
import { Measurement, NutritionLogEntry, Client, SharedFile, DataPoint, BilanTemplate, BilanResult } from '../../types';
...
const InfoRow: React.FC<{ label: string; value?: string | number }> = ({ label, value }) => (
    <div className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-client-card">
        ...
        <span className="font-semibold text-gray-800 dark:text-client-light text-right">{value || 'Non défini'}</span>
    </div>
);
...
const ClientProfile: React.FC = () => {
    ...
    const [selectedMeasurements, setSelectedMeasurements] = useState<Array<keyof MeasurementType>>(['chest']);
    const [editableWeight, setEditableWeight] = useState(user?.weight?.toString() || '');
    const [editableMeasurements, setEditableMeasurements] = useState<MeasurementType>(user?.nutrition.measurements || {});
    const [selectedBilan, setSelectedBilan] = useState<BilanResult | null>(null);

    const measurementLabels: Record<keyof MeasurementType, string> = { neck: 'Cou (cm)', chest: 'Poitrine (cm)', l_bicep: 'Biceps G. (cm)', r_bicep: 'Biceps D. (cm)', waist: 'Taille (cm)', hips: 'Hanches (cm)', l_thigh: 'Cuisse G. (cm)', r_thigh: 'Cuisse D. (cm)' };
    ...
    const measurementHistoryTable = useMemo(() => {
        ...
        const validLogs = user.nutrition.historyLog.filter(log => log.weight || (log.measurements && Object.keys(log.measurements).length > 0));
    ...
    const handleMeasurementChange = (field: keyof MeasurementType, value: string) => {
        ...
        setEditableMeasurements(prev => ({ ...prev, [field]: value === '' ? undefined : (isNaN(numValue) ? prev[field] : numValue) }));
    };

    const hasChanges = useMemo(() => {
        if (!user) return false;
        const weightChanged = editableWeight !== (user.weight?.toString() || '');
        const measurementsChanged = JSON.stringify(editableMeasurements) !== JSON.stringify(user.nutrition.measurements || {});
    ...
    const handleSaveMeasurements = () => {
        ...
        const newLogEntry: NutritionLogEntry = {
            ...
            weight: newWeight,
            calories: user.nutrition.macros ? (user.nutrition.macros.protein * 4 + user.nutrition.macros.carbs * 4 + user.nutrition.macros.fat * 9) : 0,
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col items-center">
                ...
                />
                <h2 className="mt-4 text-2xl font-bold text-gray-900 dark:text-client-light">{user?.firstName} {user?.lastName}</h2>
            ...
            <div className="space-y-2">
                ...
                <ClientAccordion title="Mes bilans">
                    {(!user?.bilans || user.bilans.length === 0) ? (
                        ...
                        <div className="space-y-3">
                            {[...user.bilans].sort((a, b) => new Date(b.assignedAt).getTime() - new Date(a.assignedAt).getTime()).map(bilan => (
                                <div key={bilan.id} className="flex flex-wrap justify-between items-center p-3 bg-client-dark rounded-lg border border-gray-700">
                                    <div>
                                        ...
                                        <p className="text-sm text-client-subtle">
                                            ...
                                            <span className={`font-medium ${bilan.status === 'completed' ? 'text-green-400' : 'text-yellow-400'}`}>
                                                {bilan.status === 'completed' ? ' Complété' : ' En attente'}
                                            </span>
                                        </p>
                                    </div>
                                    <div className="mt-2 sm:mt-0">
                                        <Button size="sm" variant="secondary" onClick={() => setSelectedBilan(bilan)} disabled={!bilan.answers && bilan.status === 'completed'}>
                ...
                <ClientAccordion title="Mensurations & Photos">
                    <h4 className="font-semibold text-lg mb-4 text-gray-900 dark:text-client-light">Graphique des Mensurations</h4>
                    ...
                    <div className="mt-4 flex flex-wrap justify-center gap-2">
                        {availableMeasurementsForSelect.map(key => (
                            <label key={String(key)} className="flex items-center space-x-2 cursor-pointer text-sm text-gray-700 dark:text-client-subtle">
                                <input
                                    type="checkbox"
                    ...
                    <div className="pt-6 mt-6 border-t border-gray-200 dark:border-client-card">
                         <h4 className="font-semibold text-lg mb-4 text-gray-900 dark:text-client-light">Historique des mensurations</h4>
                         {measurementHistoryTable.data.length > 0 ? (
                            <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-xs text-gray-500 dark:text-client-subtle uppercase bg-gray-50 dark:bg-client-dark">
                                        <tr>
                                            <th className="p-2 font-semibold sticky left-0 bg-gray-50 dark:bg-client-dark">Date</th>
                                            <th className="p-2 font-semibold">Poids (kg)</th>
                                            {measurementHistoryTable.headers.map(key => (
                                                <th key={key} className="p-2 font-semibold">{measurementLabels[key] || key}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {measurementHistoryTable.data.map((row, index) => (
                                            <tr key={index} className="bg-white dark:bg-client-card">
                                                <td className="p-2 sticky left-0 bg-white dark:bg-client-card">{row.date}</td>
                            ...
                            <p className="text-gray-500 dark:text-client-subtle text-center py-4">Aucun historique de mensurations enregistré.</p>
                        )}
                    </div>

                    <div className="pt-6 mt-6 border-t border-gray-200 dark:border-client-card">
                        <h4 className="font-semibold text-lg text-gray-900 dark:text-client-light mb-4">Enregistrer de nouvelles données</h4>
                     ...
                     <div className="pt-6 mt-6 border-t border-gray-200 dark:border-client-card">
                        <h4 className="font-semibold text-lg text-gray-900 dark:text-client-light mb-4">Mes photos de suivi</h4>
                        <input
                            type="file"
                            ref={photoInputRef}
                                ...
                                <div key={file.id} className="relative group aspect-square">
                                    ...
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex flex-col justify-between p-2 text-white">
                                        ...
                                        <div className="flex justify-between items-center">
                                            <p className="text-xs">{new Date(file.uploadedAt).toLocaleDateString('fr-FR')}</p>
                                            <button onClick={() => handleDeleteFile(file.id)} className="p-1 bg-red-500 rounded-full hover:bg-red-600">
                        ...
                        {photoFiles.length === 0 && (
                             <div className="text-center py-6">
                                <p className="text-gray-500 dark:text-client-subtle mt-1">Aucune photo téléversée.</p>
                            </div>
                        )}
                    </div>
                </ClientAccordion>
                <ClientAccordion title="Mes Documents">
                        ...
                        <div className="space-y-3">
                            {documentFiles.length > 0 ? (
                                documentFiles.map(file => (
                                    <div key={file.id} className="!bg-white dark:!bg-client-dark/50 p-3 !shadow-sm border border-gray-200 dark:border-gray-700/50 rounded-lg">
                                        <div className="flex items-center gap-4">
                                ...
                                <div className="text-center py-6">
                                    ...
                                    <p className="text-gray-500 dark:text-client-subtle mt-1">Téléversez des bilans ou autres documents à partager.</p>
                                </div>
                            )}
                        </div>
                    </div>
                 ...
                 <ClientAccordion title="Paramètres du compte">
                    <div className="flex flex-col items-start space-y-2 text-gray-700 dark:text-client-subtle">
                         <button className="hover:text-primary dark:hover:text-client-light">Modifier le mot de passe</button>
                         <button className="hover:text-primary dark:hover:text-client-light">Notifications</button>
                    </div>
                </ClientAccordion>
                        ...
                        <div className="flex items-center rounded-lg p-1 bg-gray-200 dark:bg-client-dark">
                            <button
                                ...
                                className={`px-4 py-1 text-sm font-semibold rounded-md transition-colors ${
                                    theme === 'light' ? 'bg-white text-primary shadow-sm' : 'text-gray-600 dark:text-client-subtle'
                                }`}
                            >
                                Clair
                            </button>
                            <button
                                ...
                                className={`px-4 py-1 text-sm font-semibold rounded-md transition-colors ${
                                    theme === 'dark' ? 'bg-primary text-white shadow-sm' : 'text-gray-600 dark:text-client-subtle'
                                }`}
                            >
                                Sombre
                            </button>
            ...
            {selectedBilan && bilanTemplateForModal && (
                <Modal isOpen={!!selectedBilan} onClose={() => setSelectedBilan(null)} title={selectedBilan.templateName} theme={theme} size="xl">
                    <div className="space-y-6">
                        {bilanTemplateForModal.sections.map(section => {
                            ...
                            return (
                                <div key={section.id}>
                                    <h4 className="font-semibold text-lg text-gray-800 dark:text-client-light mb-2 pt-4 border-t border-gray-200 dark:border-client-card first:pt-0 first:border-t-0">{section.title}</h4>
                                    <div className="space-y-1">
                                        {answeredFields.map(field => {
                                            const answer = selectedBilan!.answers![field.id];
                                            return (
                                                <div key={field.id} className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-client-card">
                                                    <span className="text-gray-600 dark:text-client-subtle">{field.label}</span>
                                                    <span className="font-semibold text-gray-900 dark:text-client-light text-right">{Array.isArray(answer) ? answer.join(', ') : String(answer)}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
