...
import { Client, DataPoint, WorkoutProgram, PerformanceLog, NutritionLogEntry, Measurement, NutritionPlan, MealItem, BilanResult, BilanTemplate, SharedFile, Meal, NutritionDay } from '../types';
...
import MacroStackedBarChart from '../components/charts/MacroStackedBarChart';
...
const CoachFoodJournalView: React.FC<{ client: Client }> = ({ client }) => {
    ...
    const { journalDayMeals, isFromPlan } = useMemo(() => {
        // Check for a client-logged entry first
        ...
        // Fallback to the assigned nutrition plan for that day of the week
        ...
        return { journalDayMeals: [], isFromPlan: false }; // No log and no plan
    ...
    const macroGoals = useMemo(() => {
        if (!client.nutrition.macros || (client.nutrition.macros.protein === 0 && client.nutrition.macros.carbs === 0 && client.nutrition.macros.fat === 0)) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <Button variant="secondary" size="sm" onClick={() => changeDay(-1)}>Précédent</Button>
                <span className="font-semibold text-lg text-center">{selectedDate.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                <Button variant="secondary" size="sm" onClick={() => changeDay(1)}>Suivant</Button>
            </div>
            ...
            <div className="space-y-4">
                {journalDayMeals.length > 0 ? journalDayMeals.map(meal => {
                                ...
                                <ul className="space-y-1">
                                    {meal.items.map(item => (
                                        <li key={item.id} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded">
                                            ...
                                            <span className="font-mono text-gray-700">{item.quantity}{item.unit}</span>
                                        </li>
                                    ))}
                                </ul>
...
const ClientProfile: React.FC = () => {
    ...
    const handleSaveAccess = () => {
        if (!client) return;
        const updatedClients = clients.map(c =>
            c.id === client.id ? { ...c, ...editableAccess } : c
        );
        setClients(updatedClients as Client[]);
    ...
    useEffect(() => {
        if (client && baseMetabolicData) {
            // Le TDEE de maintien est fixe et basé sur les données du client.
            setTdee(baseMetabolicData.baseTdee);

            const { protein, carbs, fat } = client.nutrition.macros;
            // Vérifie si des macros cibles ont déjà été définies.
            const areMacrosSet = protein > 0 || carbs > 0 || fat > 0;

            if (areMacrosSet) {
                // Utilise les macros cibles existantes.
                setEditableMacros(client.nutrition.macros);
                setInitialMacros(client.nutrition.macros);
            } else {
                // Si aucune macro n'est définie, initialise avec une répartition par défaut basée sur le TDEE.
                const targetTdee = baseMetabolicData.baseTdee;
                const pG = Math.round((targetTdee * 0.30) / 4); // 30% Protéines
                const fG = Math.round((targetTdee * 0.30) / 9);  // 30% Lipides
                const cG = Math.round((targetTdee * 0.40) / 4);  // 40% Glucides
    ...
    const measurementHistoryTable = useMemo(() => {
        if (!client?.nutrition.historyLog) return { data: [], headers: [] };
        const headers = new Set<keyof Measurement>();
        const validLogs = client.nutrition.historyLog.filter(log => log.weight || (log.measurements && Object.keys(log.measurements).length > 0));
                    ...
                    <Accordion title="Mes bilans">
                        {(client.bilans && client.bilans.length > 0) ? (
                            <div className="space-y-3">
                                {[...client.bilans].sort((a, b) => new Date(b.assignedAt).getTime() - new Date(a.assignedAt).getTime()).map(bilan => (
                                    <div key={bilan.id} className="flex flex-wrap justify-between items-center p-3 bg-gray-50 rounded-lg border">
                                        ...
                                        <div className="mt-2 sm:mt-0">
                                            <Button size="sm" variant="secondary" onClick={() => setSelectedBilan(bilan)} disabled={!bilan.answers}>Consulter</Button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                        ...
                                        <thead className="text-xs text-gray-500 uppercase bg-gray-50 sticky top-0">
                                            <tr>
                                                ...
                                                {measurementHistoryTable.headers.map(key => (
                                                    <th key={key} className="p-2 font-semibold">{measurementLabels[key]}</th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-200">
                                            {measurementHistoryTable.data.map((row, index) => (
                                                <tr key={index} className="bg-white hover:bg-gray-50">
                                                    ...
                                                    {measurementHistoryTable.headers.map(key => (
                                                        <td key={key} className="p-2">{row[key as keyof typeof row] || '-'}</td>
                                                    ))}
                                                </tr>
                                            ))}
                                        </tbody>
                                    ...
                                    {photoFiles.map(file => (
                                        <div key={file.id} className="relative group aspect-square">
                                            ...
                                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex flex-col justify-end p-2 text-white">
                                                <button onClick={() => handleDeleteFile(file.id)} className="absolute top-1 right-1 p-1 bg-red-500 rounded-full hover:bg-red-600">
                                    ...
                                    <div key={file.id} className="p-3 border rounded-lg flex items-center justify-between gap-2 hover:bg-gray-50">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500">{new Date(file.uploadedAt).toLocaleDateString('fr-FR')} &middot; {formatFileSize(file.size)}</p>
                                            </div>
                                        </div>
                                        <button onClick={() => handleDeleteFile(file.id)} className="p-2 text-gray-500 hover:text-red-500" aria-label="Supprimer le fichier">
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                     {coachFormations.length > 0 ? (
                                        ...
                                        <p className="text-sm text-gray-500 text-center">Vous n'avez créé aucune formation. <Link to="/app/formations" className="text-primary underline">En créer une</Link>.</p>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                <Modal isOpen={!!selectedBilan} onClose={() => setSelectedBilan(null)} title={selectedBilan.templateName} size="xl">
                    <div className="space-y-6">
                        {bilanTemplateForModal.sections.map(section => {
                            ...
                            return (
                                <div key={section.id}>
                                    <h4 className="font-semibold text-lg text-gray-800 mb-2 pt-4 border-t border-gray-200 first:pt-0 first:border-t-0">{section.title}</h4>
                                    <div className="space-y-1">
