...
const BilanTemplateManagement: React.FC = () => {
    ...
    const handleDelete = useCallback(async (templateId: string) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer ce template ? Cette action est irréversible.")) {
    ...
    const handleDuplicate = useCallback(async (template: BilanTemplate) => {
        if (!user?.id) {
        ...
        if (window.confirm(`Voulez-vous dupliquer le template "${template.name}" ?`)) {
            try {
                ...
                alert(`Erreur lors de la duplication : ${error instanceof Error ? error.message : "Erreur inconnue"}`);
            }
        }
            ...
            {coachTemplates.map(template => (
                <Card key={template.id} className="p-4 flex justify-between items-center shadow-md">
                    <div>
                        ...
                        <p className={`text-sm ${template.coachId === 'system' ? 'text-blue-600' : 'text-gray-500'}`}>
                            {template.coachId === 'system' ? 'Template Système' : 'Mon Template'}
                        </p>
                        <p className="text-xs text-gray-400">{template.sections.length} sections, {template.sections.reduce((acc, s) => acc + s.fields.length, 0)} champs</p>
                    </div>
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <Modal 
                ...
                title={viewMode === 'create' ? "Créer un Nouveau Template" : `Modifier ${selectedTemplate?.name}`}
                size="xl"
            >
