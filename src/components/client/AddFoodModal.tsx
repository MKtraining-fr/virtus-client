...
const AddFoodModal: React.FC<AddFoodModalProps> = ({ isOpen, onClose, onAddFoodItem, onAddRecipe, db, mealName }) => {
    const { theme } = useAuth();
    ...
    const categories = useMemo(() => {
        ...
        return ['Tout', ...(hasRecipes ? ['Recettes'] : []), ...(hasMeals ? ['Repas'] : []), ...sortedFoodCats];
    }, [db]);
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Ajouter à : ${mealName}`} theme={theme}>
            {!selectedFood ? (
                <div className="space-y-3">
                    <Input autoFocus type="text" placeholder="Rechercher aliment ou recette..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                        ...
                        {filteredResults.map(item => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-client-subtle pl-6">{macroString}</p>
                                </button>
                            );
                        })}
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 items-end">
                        ...
                        <div>
                             <label className="block text-sm font-medium text-gray-600 dark:text-client-subtle mb-1">Unité</label>
                             <div className="flex rounded-lg bg-gray-100 dark:bg-client-dark border border-gray-300 dark:border-gray-700">
                                <button onClick={() => setUnit('g')} className={`flex-1 py-2 rounded-l-md text-sm font-semibold transition-colors ${unit === 'g' ? 'bg-primary text-white' : 'text-gray-600 dark:text-client-subtle'}`}>g</button>
                                <button onClick={() => setUnit('ml')} className={`flex-1 py-2 rounded-r-md text-sm font-semibold transition-colors ${unit === 'ml' ? 'bg-primary text-white' : 'text-gray-600 dark:text-client-subtle'}`}>ml</button>
                            </div>
                        </div>
                    </div>
                    ...
                    {calculatedMacros && (
                        <div className="p-3 bg-gray-100 dark:bg-client-dark rounded-lg text-center">
                            <p className="font-bold text-xl text-gray-900 dark:text-client-light">{calculatedMacros.calories} kcal</p>
                            <div className="flex justify-center gap-4 mt-2 text-xs">
                                <span className="font-semibold text-red-500 dark:text-red-400">P: {calculatedMacros.protein}g</span>
                                <span className="font-semibold text-green-600 dark:text-green-400">G: {calculatedMacros.carbs}g</span>
                                <span className="font-semibold text-yellow-500 dark:text-yellow-400">L: {calculatedMacros.fat}g</span>
                            </div>
                        </div>
                    )}

                    <div className="flex justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button onClick={() => setSelectedFood(null)} className="text-gray-600 dark:text-client-subtle hover:text-gray-900 dark:hover:text-client-light px-4 py-2 rounded-lg text-sm font-semibold">Retour</button>
                        <button onClick={handleAdd} className="bg-primary text-white font-semibold px-4 py-2 rounded-lg">Ajouter</button>
                    </div>
                </div>
            )}
        </Modal>
