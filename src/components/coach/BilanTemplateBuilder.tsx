...
const BilanTemplateBuilder: React.FC<BilanTemplateBuilderProps> = ({
  initialTemplate,
  onSaveSuccess,
  onCancel,
}) => {
  const { addBilanTemplate, updateBilanTemplate, user } = useAuth();
  const [template, setTemplate] = useState<BilanTemplate>(
    initialTemplate || {
      id: uuidv4(),
  ...
  const addField = useCallback((sectionId: string) => {
    setTemplate((prev) => ({
      ...
      sections: prev.sections.map((s) =>
        s.id === sectionId ? { ...s, fields: [...s.fields, { ...defaultField, id: uuidv4() }] } : s
      ),
    }));
  }, []);
  ...
  const updateField = useCallback(
    (sectionId: string, fieldId: string, updates: Partial<BilanField>) => {
      setTemplate((prev) => ({
        ...prev,
        sections: prev.sections.map((s) =>
          s.id === sectionId
  ...
  const handleSave = async () => {
    if (isSystemTemplate) {
      alert(
        'Impossible de modifier le template système. Veuillez le dupliquer pour créer votre propre version.'
      );
      return;
    }
    ...
    try {
      ...
      alert(
        `Erreur lors de la sauvegarde du template : ${error instanceof Error ? error.message : 'Erreur inconnue'}`
      );
    } finally {
      setIsSaving(false);
    }
      ...
      <div className="border-t pt-3 mt-3 space-y-3">
        ...
        {field.conditionalOn && (
          <Input
            ...
            onChange={(e) =>
              updateField(sectionId, field.id, { conditionalValue: e.target.value || undefined })
            }
            disabled={isSystemTemplate}
          />
        )}
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <h2 className="text-2xl font-bold mb-4">
          {initialTemplate
            ? `Modifier le Template: ${initialTemplate.name}`
            : 'Créer un Nouveau Template de Bilan'}
        </h2>
        ...
        {isSystemTemplate && (
          ...
          >
            ...
            <p>
              Ce template est un modèle système et ne peut pas être modifié. Veuillez le dupliquer
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          ...
          <Button onClick={handleSave} disabled={isSaving || isSystemTemplate} isLoading={isSaving}>
