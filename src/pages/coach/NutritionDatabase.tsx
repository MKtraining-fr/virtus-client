...
const NutritionDatabase: React.FC = () => {
    ...
    const { items: sortedFoodItems, requestSort, getSortDirection } = useSortableData(foodItems, { key: 'name', direction: 'ascending' });

    const filteredData = useMemo(() => {
        if (!filter) return sortedFoodItems;
        const lowercasedFilter = filter.toLowerCase();
        return sortedFoodItems.filter(item =>
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-6">Base de Données Alimentaire (Ciqual)</h1>
            <div className="mb-4">
                <Input
                    type="text"
