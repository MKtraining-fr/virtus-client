...
const CoachShop: React.FC = () => {
  ...
  const myPartners = useMemo(
    () => partners.filter((p) => p.ownerId === user?.id),
    [partners, user]
  );
  ...
  const handleSavePartner = () => {
    if (!currentPartner || !currentPartner.name || !user)
      return alert('Le nom du partenaire est requis.');
  ...
  const handleSaveProduct = () => {
    if (!currentProduct || !currentProduct.name || !user)
      return alert('Le nom du produit est requis.');
